<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column v-if="!checkPermission(['租户'])" label="报修人" prop="username" width="110px" align="center">
        <template slot-scope="scope">
          <el-tooltip class="item" effect="light" content="点击查看租户信息" placement="right">
      ...
      <el-table-column label="维修状态" prop="status" column-key="status" width="110px" align="center" :filter-method="filterStatus" :filters="statusList" filter-placement="bottom-left">
        <template slot-scope="scope">
          <el-tooltip v-if="checkPermission(['售后人员'])" class="item" effect="light" content="点击更改修理状态" placement="right">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogApartmentVisible">
      <el-table v-if="apartmentinfo!=null" v-loading="apartmentLoading" :data="[apartmentinfo]" border fit highlight-current-row>
          ...
          <template slot-scope="props">
            <el-form label-position="left" inline class="demo-table-expand">
              <el-form-item label="户型">
                <el-tag type="info">{{ props.row.houseType }}</el-tag>
              </el-form-item>
              <el-form-item label="面积">
                <el-tag>{{ props.row.roomArea }}m&sup2;</el-tag>
              </el-form-item>
              <el-form-item label="朝向">
                <el-tag type="success">{{ props.row.face }}</el-tag>
              </el-form-item>
        ...
        <el-table-column prop="roomArea" label="面积(平方米)" />
        <el-table-column prop="face" label="朝向" />
      </el-table>
      <div slot="footer" class="dialog-footer">
...
<script>
import { fetchList, updateRepair, getImages, image } from '@/api/repairs'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      if (this.checkPermission(['售后人员'])) {
        this.listQuery.repairId = this.userInfo.repairId
      }
      if (this.checkPermission(['租户'])) {
